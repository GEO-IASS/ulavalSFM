...
def create_submit(nc, walltime, status):
     ...
     wall2 = "#PBS -l walltime=" + str(int(walltime)*2) + " # Durée en secondes\n"
...
def sift_images(images, cores, verbose=False, force_rebuild=False):
    ...
    if verbose:
        print("Those images haven't been searched for sift points yet :")
        print(todo)

    if todo == images or force_rebuild:
        if verbose:
            print("Doing all the researches :")
        subprocess.call(["mpirun", "-n", str(cores), "../cDoSift", "./"], shell=False, env=env)
    else:
        print("Passing the sift research")


def match_images(cores, verbose=False, force_rebuild=False):

    env = dict(os.environ)

    if not force_rebuild and os.path.isfile("matches.init.txt"):
